...
def prep_data(df):
    ...
    gp = df[['device', 'app', 'hour', 'channel']].groupby(by=['device', 'app', 'hour'])[['channel']].count().reset_index().rename(index=str, columns={'channel': 'ndev_app_hh'})
    df = df.merge(gp, on=['device','app','hour'], how='left')
    df['ndev_app_hh'] = df['ndev_app_hh'].astype('uint32')
    del gp
    gc.collect()
...
if TRAIN:
    ...
    train_df = pd.read_csv(train_path, skiprows=skiprows, nrows=nrows,dtype=dtypes, usecols=train_cols)
